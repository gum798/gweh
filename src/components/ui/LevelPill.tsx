...
//
// 색은 게이트가 검증하는 잉크/틴트 쌍만 쓴다. 초안은 배경을 잉크의 10% 알파로
// 깔았는데, 그건 -light 틴트가 아니라서 scripts/check-contrast.mjs 가 검사하지
// 않는 조합이었다. 다섯 줄 전부 쌍으로 맞춰 두면 팔레트가 통째로 바뀌어도
// 게이트가 다섯 줄을 전부 따라온다 — 다크 전환(Task 7)이 실제로 그렇게 됐고,
// 지금 값은 잉크가 밝고 틴트가 어둡다:
//   잉크 위 틴트  success 6.29 / warning 6.51 / danger 5.77 / info 6.07
//   틴트 위 gal-black  네 색 모두 10.1 이상
// 중립 등급의 면만 카드와 겹치지 않도록 페이지색 우물로 내렸다(카드 대비 1.33:1).
